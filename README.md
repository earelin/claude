...
| [`backend`](plugins/backend) | A `code-reviewer` agent and a `refactoring` agent for reviewing and refactoring backend changes on the current branch or a pull request, plus `java-unit-test`, `java-integration-test`, and `java-acceptance-test` skills for writing Java tests. |
