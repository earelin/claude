...
- **Agent** — `refactoring`, a principal platform engineer that iteratively improves the
  internal structure of the pipeline and infrastructure code a PR or branch adds or modifies
...
Ask Claude to "review this pipeline change" or "review this infrastructure PR" to invoke the
`code-reviewer` agent, or "refactor the infrastructure code of this branch" to invoke the
`refactoring` agent.
